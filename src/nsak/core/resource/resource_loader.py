...
class ResourceLoader[T]:
    ...
    def get_search_paths(cls) -> set[Path]:
        ...
        """
        resource_path = f"{cls.ResourceClass.__name__.lower()}s"
    ...
    def _load(cls, path: Path) -> dict[str, Any]:
        ...
        """
        resource_type_name = cls.ResourceClass.__name__
    ...
    @classmethod
    def _to_resource(cls, data: dict[str, Any], path: Path) -> T:
        """
    ...
    @classmethod
    def _safe_to_resource(cls, data: dict[str, Any], path: Path) -> T:
        """
        Wraps `_to_resource` for generic error handling.
        """
        try:
            return cls._to_resource(data, path)
    ...
    def load_all(cls) -> list[T]:
        ...
        for search_path in cls.get_search_paths():
            for path in search_path.iterdir():
                ...
                try:
                    data = cls._load(path)
                    resource = cls._safe_to_resource(data, path)
