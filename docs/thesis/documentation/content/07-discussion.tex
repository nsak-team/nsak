...
The requirements to not expose the still in use BFH Network Security Lab limits to a narrow discussion of the results.
...
\textit{qwen3:30b} performs host and service discovery correctly but lacks the enumeration depth for Critical finding.
...
In regards of the operator support, the \gls{AI} scenario differs most from the static baseline.
Instead of a raw \textit{NetworkDiscoveryResultMap}, the frontier model returns an assessment that ranks findings by
...
agent to gather information rather than a finding comparison.
...
dominates the cost of single-agent reconnaissance for smaller models or more complicated environments which results ultimately
also in larger context.
