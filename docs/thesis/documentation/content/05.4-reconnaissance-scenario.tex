...
For local network segments, it runs \mintinline{bash}{arp-scan --localnet} on the given interface, which sends \gls{ARP} requests and collects the \gls{IP} address, \gls{MAC} address, and vendor of each responding host.
If an explicit subnet is provided, the drill falls back to \mintinline{bash}{nmap -sn} instead, which uses \gls{ICMP} and \gls{TCP} probes and therefore works across Layer~3 boundaries.
\gls{MAC} addresses are only available for hosts on the same Layer~2 segment; remote hosts are recorded without a \gls{MAC}.
Further limitations and possible extensions of the drill, such as supporting a static \gls{IP} address in networks without \gls{DHCP}, are discussed in the sprint review~\ref{par:milestone-review-network-discovery-red-team}.
...
The drill exposes two arguments: a required \mintinline{bash}{interface} and an optional \mintinline{bash}{subnet}.
If \mintinline{bash}{subnet} is omitted, the drill defaults to a local ARP scan; if provided, it switches to the nmap-based subnet scan.
...
The \textit{Port Scan} drill receives the \mintinline{bash}{ReconnaissanceScenarioResult} produced by host discovery and runs \mintinline{bash}{nmap -sV --open} against each discovered \gls{IP}.
For every open port, the drill records the port number, transport protocol (\gls{TCP}/\gls{UDP}), service name, and detected version string and appends a \mintinline{bash}{NetworkService} entry to the existing result map.
...
Well-known services such as \gls{HTTP}, \gls{DNS}, \gls{FTP}, \gls{SMTP}, \gls{SMB}, and \gls{LDAP} are mapped to a specific set of scripts (e.g.\ \mintinline{bash}{http-title}, \mintinline{bash}{dns-zone-transfer}, \mintinline{bash}{smb-security-mode}).
Unknown services fall back to the generic \mintinline{bash}{banner} script, which captures the initial response of any \gls{TCP} service.
The drill returns a mapping of \mintinline{bash}{ip:port} keys to lists of finding strings extracted from the \gls{NSE} output.
...
The primary output of the scenario is the \mintinline{bash}{ReconnaissanceScenarioResult},
which aggregates all results per interface.
Each entry maps an interface name to a \mintinline{bash}{NetworkDiscoveryResult} containing the list of discovered \mintinline{bash}{NetworkService} objects with their endpoints (IP, MAC, port, protocol, version).
After port scanning, the scenario prints the result as a human-readable table.
The service enumeration findings are printed separately as a structured list grouped by \mintinline{bash}{ip:port}.
