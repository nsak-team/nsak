...
To avoid loading the configuration on import, the module-level config object is wrapped in a \mintinline{python}{lazy_object_proxy.Proxy}.
...
In the context of \enquote{Project 2}\ref{sec:predecessor-project-project-2}, this resource type was already defined conceptually,
and the~\gls{BananaPI R4} was added as an example in the library under \mintinline{bash}{lib/devices/bananapi_r4/}.
The resource type was not actually added to the \gls{NSAK} core,
so the Device resource type has to be implemented in the context of this project.
...
In \enquote{Project 2}\ref{sec:predecessor-project-project-2}, it became already evident that the described classes contain a lot of boilerplate code between each other.
...
Currently, the focus is only on network configuration, as this is the most important information for the scenarios and drills.
...
The code snippet, located under \mintinline{bash}{src/nsak/core/device/device_loader.py}~\ref{lst:device-loader-configuration-parsing}
shows how the \gls{YAML} configuration gets parsed to the respective datastructures defined in\\
\mintinline{bash}{src/nsak/core/network/configuration.py}
and~\mintinline{bash}{src/nsak/core/device/device.py}~\ref{lst:device-configuration-datastructures}.
...
As stated in Section~\ref{subsubsec:device-configuration-management-implementation}, all resource classes were refactored to inherit from base classes (Table~\ref{tab:device-resource-classes}).
...
so that all \gls{YAML} resources can be merged into a single data structure without key conflicts.
