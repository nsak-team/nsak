...
The configuration management in \gls{NSAK} is organised into three distinct layers, each serving a different scope and lifetime.

The \textbf{static configuration} layer resolves fixed filesystem paths at import time.
It derives all relevant paths from a single base directory, which can be overridden via an environment variable to support different deployment contexts such as \gls{Docker} containers or automated test environments.

The \textbf{runtime configuration} layer holds mutable state that persists across \gls{CLI} invocations.
It is loaded from a \gls{YAML} file on every command execution and written back whenever state changes, for example, when a different device is loaded.
This layer serves as the single source of truth for any information that must be shared between separate \gls{CLI} calls.

The \textbf{resource configuration} layer is defined by the resource library itself.
Each resource type \textendash devices, drills, scenarios, and environments \textendash declares its configuration inside a \gls{YAML} file stored in the library.
For devices, this includes network interface assignments, while drills and scenarios additionally declare their typed argument interfaces, which the \gls{CLI} uses to generate the correct command options at runtime.

\todo{This section is repeating what's already stated in the concepts chapter...}
...
Since the root \gls{CLI} group imports \mintinline{bash}{config} on every command invocation (listing~\ref{lst:nsak-cli-init}), this ensures the configuration is always initialised before any command logic runs.
...
The following excerpt~\ref{lst:device-resource-configuration-yaml-specification} shows the specification for the configuration section of the device resource \gls{YAML}.
...
The following listing shows the newly available \gls{CLI} commands for managing device configurations~\ref{lst:device-configuration-cli-management}:
...
The \mintinline{bash}{discover_hosts} drill in listing~\ref{lst:discover-hosts-yaml} illustrates this: its single \mintinline{bash}{interface} argument of type \mintinline{bash}{str} is translated directly into a \mintinline{bash}{--interface} option on the generated \gls{CLI} command.
