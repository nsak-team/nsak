...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{l c c c c c c}
            ...
            Unstructured (claude-opus-4-7) & 739 s          & 222,094      & --          & --             & --             & 6/109\textsuperscript{*} \\
            Multi-Agent (qwen3:8b, local)  & ---            & ---          & --          & --             & --             & 0/6                      \\
            Static Scenario                & 1780 s         & ---          & --          & --             & --             & 1/1\textsuperscript{**}                      \\
            \hline
        \end{tabular}
    }
...
The low \(6/109\) success factor of the unstructured configuration is mostly an infrastructure artifact rather than a model limitation:
103 of the 109 attempts aborted instantly because the \gls{Anthropic} \gls{API} usage limit was reached, as the model's guardrail monthly quota got triggered mid-benchmark.
All six runs that actually reached the model succeeded.
The static scenario is fully deterministic and produces identical results on every execution, therefore only one run was recorded instead of repeating it multiple times like the \gls{AI} configurations.\\
...
requires 739\,s \textthreequartersemdash more than twice as long.
The extended duration of the unstructured version is due to its tendency to produce lengthy
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{@{}lccl@{}}
            ...
            \midrule
            claude-opus-4-7 (multi-agent)  & 7                    & 2                      & All major structural risks found; \gls{SNMP}, open \gls{DNS} recursion and cloned keys missed \\
            claude-opus-4-7 (unstructured) & 9                    & 2                      & All three discriminating findings surfaced; ESXi host discovered                              \\
            qwen3:8b (local, multi-agent)  & 1                    & N/A                    & Zero \gls{CLI} calls; no reconnaissance executed                                              \\
...
artifacts.
The main limitation of this configuration is its very low success rate (5.5\,\%)\footnote{\label{fn:nslab-quota}%
This rate is largely an infrastructure issue or handling issue rather than a measure of reconnaissance quality.
All 103 failed attempts aborted instantly with an identical HTTP~400 \texttt{BadRequestError}
(\enquote{You have reached your specified API usage limits}). Because the benchmark loop retries until ten
\emph{successful} runs are collected without capping the number of attempts. All six runs that actually
reached the model -- five before the quota wall and one after -- succeeded.}
and the substantially higher token cost (mean 222,094 tokens), driven by the unstructured reasoning
