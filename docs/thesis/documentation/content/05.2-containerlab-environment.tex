...
To evaluate reconnaissance scenarios, common services must run on endpoints. \gls{BIND} is a complete implementation of the \gls{DNS} protocol that can be configured through the \mintinline{bash}|named.conf| file as an authoritative name server and resolver~\cite{bind9}.
...
For reconnaissance purposes, several vulnerabilities and flaws were introduced, and presented in detail in Listing~\ref{lst:containerlab-topology-yaml-overview}.
...
For simplicity, bash scripts were used to configure and start the services,
