...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{../assets/diagrams/ai-variants/variant1-internal-agent.drawio}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{../assets/diagrams/ai-variants/variant2-external-ai-client.drawio}
...
The project team selected \textbf{Variant 1: AI Agent in NSAK}~\ref{subsec:variant-decision-ai-integration-variant-1}, as it significantly enhances the autonomy and automation capabilities of the \gls{NSAK} framework.

From a technical perspective, embedding the agent directly within the \gls{NSAK} device enabled the framework to serve as an \gls{AGENTIC-AI} harness.
Furthermore, the agent can leverage existing drills and scenarios as reusable building blocks.

For tool calling, the team decided against relying directly on \gls{MCP}.
A large number of tools bloat the context and pollute the quality of the result.
Instead, \textit{LangChain Tools} were used.
This approach preserved modularity while allowing the agent's toolset to be integrated with the \gls{NSAK} framework.
