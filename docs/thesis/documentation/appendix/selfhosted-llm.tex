...
act as reverse proxy for the \enquote{Open WebUI} and the Ollama API under the directory \enquote{/llm/}.
...
\newpage
\begin{lstlisting}[language=bash,caption={Local LLM System Setup},label={lst:llm-system-setup}]
...
\newpage
\begin{lstlisting}[language=bash,caption={/home/ai/docker-compose.yml},label={lst:nginx-openwebui-docker-compose-yml}]
services:
  swag:
...
\newpage
\begin{lstlisting}[language=bash,caption={swag/nginx/site-confs/default.conf},label={lst:nginx-default-conf}]
...
\begin{lstlisting}[language=bash,caption={Restart SWAG Container},label={lst:restart-swag-container}]
...
Later on we can use the Ollama API served under \enquote{/llm/} as a backend for \glsxtrshort{AI} agents.
