...
\textbf{Goals (Must):}
\begin{itemize}
    \item[$\square$] The scenario identifies the available ports and interfaces on the device
    \item[$\square$] The scenario discovers the available subnets and obtains valid \glsxtrshort{IP} addresses
    \item[$\square$] The scenario discovers \glsxtrshort{MAC} addresses, \glsxtrshort{IP} addresses and services (port scan)
    \item[$\square$] The scenario supports at least a fast full-search mode
    \item[$\square$] The scenario returns a network map as a structured data type
\end{itemize}
