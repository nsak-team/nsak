...
Based on the vaguely described minimum requirements in the section Devices~\ref{subsec:devices}, we defined the following requirements for the hardware platform used in this project:
\begin{itemize}
...
\section{Evaluated Boards}\label{sec:evaluated-boards}
...
\section{Hardware Selection}\label{sec:hardware-selection}

Based on the defined requirements and the evaluation of alternatives, compared in table~\ref{tab:reqcheck}, the~\textbf{Banana Pi R4} and the~\textbf{NanoPI R76S} are the most suitable hardware platforms for this prototype implementation.
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{|p{6cm}|c|c|c|c|c|c|c}
            ...
            \hline
            \scriptsize \geq 2 native Ethernet interfaces     & \ding{51}               & \ding{51}            & \ding{51}                & \ding{51} & \ding{55} & \ding{55} & \ding{51} \\
            \hline
...
\end{table}
The \textbf{Banana Pi R4} offers two native 2.5~GbE interfaces for inline sniffing the board is compact, affordable, and supported by a strong community.
...
The \textbf{NanoPi R76S} is more compact and provides up to 16GB of RAM, which is advantageous for memory-intensive processing and buffering tasks.
...
Alternative boards such as the \textbf{Banana Pi R3 Mini}, R3 are limited overall performance.
\textbf{PI 4 or 5} offer higher single core performance but were ultimately discarded because they provide only a single native Ethernet interface, requiring external adapters that reduce performance for inline sniffing scenarios.
